...
function dominantDirection(text) {
  let dist = textScripts(text);
  let maxVal = dist.reduce((currMax, script)=> {
    return script.count > currMax.count ? script:currMax;
  }, {name:"None", count:0})

  if (maxVal.name == "None") return "NA"
  return SCRIPTS.find((s)=>s.name==maxVal.name).direction;
}
